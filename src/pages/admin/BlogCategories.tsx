...
const BlogCategories = () => {
  ...
  const { data: categories, loading, saving, upsertData, deleteData, deleting } = useAdminCRUD<BlogCategory>({
  ...
  const handleBulkStatus = async (active: boolean) => {
    ...
    try {
      ...
      toast.success(t(`Updated ${selectedIds.length} categories.`, `${selectedIds.length}件のカテゴリーを更新しました。`, `Đã cập nhật ${selectedIds.length} danh mục.`));
  ...
  const handleBulkDelete = async () => {
    if (!selectedIds.length) return;
    if (!window.confirm(t(`Delete ${selectedIds.length} categories?`, `${selectedIds.length}件のカテゴリーを削除しますか？`, `Xóa ${selectedIds.length} danh mục?`))) return;
    ...
    try {
      const { error } = await supabase
        ...
        .in("id", selectedIds);
      if (error) throw error;
      toast.success(t(`Deleted ${selectedIds.length} categories.`, `${selectedIds.length}件のカテゴリーを削除しました。`, `Đã xóa ${selectedIds.length} danh mục.`));
      queryClient.invalidateQueries({ queryKey: ["blog_categories"] });
      setSelectedIds([]);
    } catch {
  ...
  return (
    <AdminLayout>
      ...
      <div className="space-y-10 animate-in fade-in duration-700 pb-12">
        <AdminPageHeader
          title={translations[lang].blogCategories}
          description="Manage taxonomy nodes for granular content classification."
          searchTerm={searchTerm}
          onSearchChange={setSearchTerm}
          primaryAction={{
            label: t("Add Category", "カテゴリーを追加", "Thêm danh mục"),
            onClick: handleAdd
          }}
          headerActions={
            <AnimatePresence>
        ...
        <div className="bg-white/40 backdrop-blur-xl border border-white/40 rounded-[2.5rem] md:rounded-[3rem] overflow-hidden shadow-sm">
          <ResponsiveDataTable
            ...
            emptyState={{
              title: t("No blog categories found.", "ブログカテゴリーが見つかりません。", "Không tìm thấy danh mục blog nào."),
              icon: LayoutGrid,
        ...
        <AdminDialogForm
          ...
          title={editingId ? t("Edit Category", "カテゴリーを編集", "Sửa danh mục") : t("Add Category", "カテゴリーを追加", "Thêm danh mục")}
          description={t("Define the classification for your blog entries.", "ブログ記事の分類を定義します。", "Định nghĩa phân loại cho các bài viết blog của bạn.")}
          tabs={taxonomyTabs}
          activeTab={activeTab}
...
export default memo(BlogCategories);
